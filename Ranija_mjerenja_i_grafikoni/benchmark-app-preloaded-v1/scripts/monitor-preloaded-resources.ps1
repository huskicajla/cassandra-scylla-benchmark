...
$preloadedStateFile = Join-Path $resultsRoot 'preloaded-state-v1.csv'
$resourceFile = Join-Path $resultsRoot 'resource-usage-v1.csv'
$logFile = Join-Path $resultsRoot 'OVERNIGHT-EXECUTION-LOG.txt'
...
function Get-ActiveExperiment {
    ...
    $activeExperiment = $latest.Values |
            Where-Object { $_.event_type -eq 'STARTED' } |
            Sort-Object event_at_utc -Descending |
            Select-Object -First 1
    if ($null -ne $activeExperiment) {
        return $activeExperiment
    }

    if (Test-Path -LiteralPath $preloadedStateFile) {
        $latestState = @(Import-Csv -LiteralPath $preloadedStateFile |
                Select-Object -Last 1)
        if ($latestState.Count -gt 0 -and
                $latestState[0].event_type -eq 'BASELINE_SEED_STARTED') {
            $seedStarted = [DateTime]::Parse($latestState[0].recorded_at_utc).ToUniversalTime()
            if (([DateTime]::UtcNow - $seedStarted).TotalMinutes -gt 45) {
                return $null
            }
            return [pscustomobject]@{
                experiment_id = $latestState[0].experiment_id
                database = $latestState[0].database
            }
        }
    }
...
if (-not (Test-Path -LiteralPath $resourceFile) -or
        (Get-Item -LiteralPath $resourceFile).Length -eq 0) {
...
while ([DateTime]::UtcNow -lt $deadline) {
    try {
        ...
        foreach ($rawStat in @($rawStats)) {
            ...
            if ($stat.Name -notmatch '^(cassandra|scylla)(-scale1)?-node[1-3]$') {
                continue
            }
