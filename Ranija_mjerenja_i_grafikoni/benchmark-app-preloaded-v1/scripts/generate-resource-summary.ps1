...
$firstResourceLine = Get-Content -LiteralPath $resourceFile -TotalCount 1
if ($firstResourceLine -like 'captured_at_utc,*') {
    $resourceRows = @(Import-Csv -LiteralPath $resourceFile)
} else {
    $resourceRows = @(Import-Csv -LiteralPath $resourceFile -Header $resourceHeader)
}
...
foreach ($id in $ExperimentId | Select-Object -Unique) {
    ...
    $lines = @(
        ...
        ''
        'DATABASE CONTAINERS (all active database nodes combined)'
