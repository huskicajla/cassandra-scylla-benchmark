...
try {
    ...
    foreach($cell in $selected) {
        ...
        if($activeCompose -ne $compose) {Stop-ResearchClusters}
        ...
        try {
            if($activeCompose -eq $compose){$status=Wait-ResearchNodes $compose $cell.nodes}
            else {$status=Start-ResearchNodes $compose $cell.nodes;$activeCompose=$compose}
        }
        catch {
